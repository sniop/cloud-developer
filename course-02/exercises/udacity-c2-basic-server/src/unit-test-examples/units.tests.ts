...
describe('concat', () => {
  ...
  it('should fail if string one is blank', () => {
    expect(()=>{ concat('','stringtwo') }).to.throw('empty string')
  });

  it('should fail if string two is blank', () => {
    expect(()=>{ concat('stringone','') }).to.throw('empty string')
  });
